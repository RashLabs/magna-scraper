...
def _run_stage(stage: str, cancel_check, progress_cb,
    ...
    company_ids = scrape_kwargs.get("company_ids")
    if stage == "scrape":
        ...
        run(reprocess=reprocess, since=since, company_ids=company_ids,
        ...
        run(headless=scrape_kwargs.get("headless", True), reprocess=reprocess, since=since,
            company_ids=company_ids, cancel_check=cancel_check, progress_cb=progress_cb)
    elif stage == "extract":
